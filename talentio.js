...
// ]
function volleyMatch(p1, p2) {
    var score1 = 0;
    var score2 = 0;
    var scoresActuel = "".concat(score1, "-").concat(score2);
    var scoresPasse = "";
    var scoresTab = [scoresActuel];
    var nbSet1 = 0;
    var nbSet2 = 0;
    //scénarios équipe 1 ou équipe 2 sert
    function service1(max) {
        if (Math.random() >= p1) {
            score2 += 1;
            scoresActuel = "".concat(score1, "-").concat(score2);
            scoresTab.push("".concat(scoresPasse, " ").concat(scoresActuel));
            if (!verifVictoire2(max)) {
                service2(max);
            }
            else {
                scoresPasse += "".concat(scoresActuel, " ");
                score1 = 0;
                score2 = 0;
                scoresActuel = "".concat(score1, "-").concat(score2);
                scoresTab.push("".concat(scoresPasse, " "));
                nbSet2 += 1;
            }
        }
        else {
    ...
    //vérif fin de sets
    function verifVictoire1(max) {
        return score1 >= max ? (score1 - score2 >= 2 ? true : false) : false;
    }
    function verifVictoire2(max) {
